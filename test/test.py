...
ref_adata = sc.read('/home/cxc/code/TOSICA/TOSICA/data/demo_train.h5ad')
ref_adata = ref_adata[:,ref_adata.var_names]
query_adata = sc.read('/data1/home/chenxc/data/demo_test.h5ad')
query_adata = query_adata[:,ref_adata.var_names]

# adata = sc.read_h5ad('/home/cxc/code/scBERT-master/data/data/Zheng68K.h5ad')
# del adata.obs['TSNE.1']
# del adata.obs['TSNE.2']
# del adata.obs['n_genes']
# del adata.uns
# train_adata, valid_adata = get_KFold_data(5, 1, adata)

# Divide training and testing sets,80% as training set,20% as test set
# arr = np.arange(adata.n_obs)
# np.random.seed(0)
# np.random.shuffle(arr)
# train_adata = adata[arr[:int(adata.n_obs*0.95)]]
# test_adata = adata[arr[int(adata.n_obs*0.95):]]

#
TOSICA.train(ref_adata, gmt_path=None, label_name='Celltype',epochs=10,project='demo_swin_trans_1_layer_8_blocks_8batch_new', batch_size=8, max_gs=256,window_size=8)

correct = []
for i in range(9):
    model_weight_path = '/home/cxc/code/TOSICA-2layer-2/test/demo_swin_trans_1_layer_8_blocks_8batch_new/model-%d.pth' %(i)
    new_adata = TOSICA.pre(query_adata, model_weight_path=model_weight_path, project='demo_swin_trans_1_layer_8_blocks_8batch_new',batch_size=8, max_gs=256,window_size=8)
    correct_sum = 0
    Prediction = new_adata.obs['Prediction']
    cell = query_adata.obs['Celltype']
    del query_adata.obs['Prediction']
    del query_adata.obs['Probability']
    ...
    correct.append(correct_sum/new_adata.n_obs)
    if i == 9:
        new_adata.write('/home/cxc/code/TOSICA-2layer-2/test/demo_swin_trans_1_layer_8_blocks_8batch_new/pre_adata.h5ad')
    #print(str(i) + ' correct : ' + str(correct_sum/new_adata.n_obs))
