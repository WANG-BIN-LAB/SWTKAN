...
from .TOSICA_model import SwinTransformer
...
def splitDataSet(adata2,label_name='Celltype', tr_ratio= 0.7):
    """ 
    Split data set into training set and test set.

    """
    label_encoder = LabelEncoder()
    el_data = pd.DataFrame(todense(adata2), index=np.array(adata2.obs_names).tolist(), columns=np.array(adata2.var_names).tolist())
    el_data[label_name] = adata2.obs[label_name].astype('str')
    # del adata2  # Cleaning up useless variables to prevent memory benefits
    # gc.collect()
...
def get_gmt(gmt):
    ...
    gmt_files = {
        "c2": [root / "resources/c2.all.v2023.2.Hs.symbols.gmt"],
        "c3": [root / "resources/c3.all.v2023.2.Hs.symbols.gmt"],
...
def fit_model(adata, gmt_path, project = None, pre_weights='', label_name='Celltype',max_g=300,max_gs=300, mask_ratio = 0.015,n_unannotated = 1,batch_size=8, embed_dim=48,depth=2,num_heads=4,lr=0.001, epochs= 10, lrf=0.01,window_size=8):
    GLOBAL_SEED = 1
    ...
    exp_train, label_train, exp_valid, label_valid, inverse,genes = splitDataSet(adata2=adata, label_name=label_name)
    if gmt_path is None:
    ...
    valid_loader = torch.utils.data.DataLoader(valid_dataset,
                                             ...
                                             shuffle=False,
                                             pin_memory=True,drop_last=True)
    # model = create_model(num_classes=num_classes, num_genes=len(exp_train[0]),  mask = mask,embed_dim=embed_dim,depth=depth,num_heads=num_heads,has_logits=False,max_gs=max_gs).to(device)
    model = SwinTransformer(num_genes=len(exp_train[0]), token=max_gs,embed_dim=embed_dim,window_size=window_size,device=device,num_classes=num_classes)
    ...
    # para_sum = sum(dict((p.data_ptr(), p.numel()) for p in model.parameters() if p.requires_grad).values())
    # print(f"total parameters : {para_sum}")
    print('Model builded!')
    ...
    for epoch in range(epochs):
        train_loss, train_acc = train_one_epoch(model=model,
                                                ...
                                                device=device,
                                                epoch=epoch)
        # print("[train epoch {}]  loss:  {:.4f},  acc:  {:.4f} ".format(epoch, train_loss, train_acc))
        scheduler.step() 
        val_loss, val_acc = evaluate(model=model,
                                     data_loader=valid_loader,
                                     device=device,
                                     epoch=epoch)
        # print("[valid epoch {}]  loss:  {:.4f},  acc:  {:.4f} ".format(epoch, val_loss, val_acc))
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'valid'], loc='upper left')
    plt.savefig('./'+project+'/accuracy.png')
    plt.show()
    print(f"The Pre-Trained model saved at  :  {project_path}/model-{epochs-1}.pth")
