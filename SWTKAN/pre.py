...
def prediect(adata,model_weight_path,project,mask_path,laten=False,save_att = 'X_att', save_lantent = 'X_lat',n_step=10000,cutoff=0.001,n_unannotated = 1,batch_size = 8,embed_dim=48,depth=2,num_heads=4,max_gs=200,window_size=8):
    ...
    # model = create_model(num_classes=n_c, num_genes=num_genes,mask = mask, has_logits=False,depth=depth,num_heads=num_heads,max_gs=max_gs).to(device)
    model = SwinTransformer(num_genes=num_genes, token=max_gs,embed_dim=embed_dim,window_size=window_size,device=device,num_classes=n_c)
    ...
    while (n_line) <= all_line:
        ...
        with torch.no_grad():
            ...
            for step, data in enumerate(data_loader):
                ...
                pre_classes.extend(pre_class)
                predict_classes.extend(predict_class)
                # if laten:
    ...
    # print(all_line)
    new = adata
    new.obs.insert(loc=1, column='Prediction', value=predict_classes)
    new.obs.insert(loc=2, column='Probability', value=pre_classes)
