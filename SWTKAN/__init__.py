...
def train(adata, gmt_path=None, project=None,pre_weights='', label_name='Celltype',max_g=300,max_gs=200,mask_ratio =0.015, n_unannotated = 1,batch_size=8, embed_dim=16,depth=2,num_heads=4,lr=0.001, epochs= 10, lrf=0.01,window_size=8):
    r"""
    ...
    fit_model(adata, gmt_path, project=project,pre_weights=pre_weights, label_name=label_name,
              ...
              embed_dim=embed_dim,depth=depth,num_heads=num_heads,lr=lr, epochs= epochs, lrf=lrf,window_size=window_size)


def pre(adata,model_weight_path,project,laten=False,save_att = 'X_att', save_lantent = 'X_lat',n_step=10000,cutoff=0.1,n_unannotated = 1,batch_size=8,embed_dim=16,depth=2,num_heads=4,max_gs=200,window_size=8):
    r"""
    ...
    save_lantent
        The name of the laten matrix to be added in the adata.obsm.y sin
ight_
path
h
path
    ...
    adata = prediect(adata, model_weight_path, project=project, mask_path=mask_path, laten=laten, save_att=save_att,
                     save_lantent=save_lantent, n_step=n_step, cutoff=cutoff, n_unannotated=n_unannotated,
                     batch_size=batch_size, embed_dim=embed_dim, depth=depth, num_heads=num_heads,max_gs=max_gs,window_size=window_size)
    return(adata)
